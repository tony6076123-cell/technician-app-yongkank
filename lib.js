...
async function readFirebase(collection) {
  ...
  return allDocs.map(d => {
    ...
    if (obj.location !== undefined || obj.role !== undefined) {
      const seg = obj._id.split('_');
      const okLoc = s => s === '永康' || s === '歸仁';
      const okRole = s => s === '技師' || s === '接待';
      if (obj.location !== undefined && !okLoc(obj.location) && okLoc(seg[0])) obj.location = seg[0];
      if (obj.role !== undefined && !okRole(obj.role) && okRole(seg[1])) obj.role = seg[1];
    }
    return obj;
  });
}
...
// 本機版透過 seed-technicians.js 手動跑；雲端版由 server.js 開機時自動偵測空集合觺發，兩邊共用同一份邏輯。
function buildSeedDocs(location) {
  ...
  const managerDoc = {
    id: 'manager', name: `(待補)${location}廢長`, location, dept: '', role: '廢長', title: '廠長',
